    ...
    <form onSubmit={handleSubmit} className="relative">
      <div className="flex gap-4">
        <div className="flex flex-col items-center gap-2">
          <div className="w-12 h-12 rounded-full bg-secondary/20 overflow-hidden border border-border/10">
            <img
          ...
          <div className="w-0.5 flex-1 bg-border/20 rounded-full min-h-[40px]" />
        </div>

        <div className="flex-1 space-y-4">
          <div className="space-y-1">
            <p className="text-sm font-bold text-foreground">{isAnonymous ? 'Anonymous Architect' : profile?.displayName}</p>
            <textarea
              autoFocus
              value={content}
              onChange={e => setContent(e.target.value)}
              onPaste={handlePaste}
              className="w-full bg-transparent border-none p-0 text-base text-foreground placeholder:text-muted-foreground/40 focus:ring-0 transition-all resize-none min-h-[100px]"
          ...
          <div className="flex flex-wrap gap-2">
            {existingImageUrls.map((url, idx) => (
              <div key={`existing-${idx}`} className="relative group">
                <img src={url} alt="Preview" className="h-32 rounded-[12px] border border-border/20 object-cover" />
                <button
                  type="button"
            ...
            {imageFiles.map((file, idx) => (
              <div key={`new-${idx}`} className="relative group">
                <img src={URL.createObjectURL(file)} alt="Preview" className="h-32 rounded-[12px] border border-border/20 object-cover" />
                <button
                  type="button"
            ...
            {gifUrl && (
              <div className="relative group">
                <img src={gifUrl} alt="GIF Preview" className="h-32 rounded-[12px] border border-border/20 object-cover" />
                <button
                  type="button"
          ...
          <div className="flex items-center justify-between pt-4">
            <div className="flex gap-4 text-muted-foreground/60">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1 rounded-full transition-all border ${
                  isAnonymous ? 'bg-primary/10 border-primary/20 text-primary' : 'text-muted-foreground/60 hover:text-foreground border-transparent hover:bg-secondary/20'
                }`}
              >
            ...
            <button
              ...
              type="submit"
              className="px-6 py-2 bg-foreground text-background font-bold rounded-full hover:opacity-90 active:scale-95 transition-all disabled:opacity-30 disabled:scale-100 flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
      ...
      <AnimatePresence>
        {showGifPicker && (
          <motion.div
            ...
            className="gif-picker-container absolute z-50 bottom-16 left-0 bg-card border border-border/20 p-2 rounded-[16px] shadow-2xl w-72 h-80 overflow-y-auto flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-center mb-2 px-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Trending GIFs</span>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              {gf ? (
                ...
                <div className="text-[10px] text-muted-foreground p-4 text-center">Giphy API key not configured.</div>
              )}
            </div>
          </motion.div>
        )}
