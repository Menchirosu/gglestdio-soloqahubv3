...
// Typewriter runs only on the user's first visit (session-scoped)
const _hasShownTypewriter = { current: sessionStorage.getItem('tw_shown') === '1' };

function TypewriterText({ text, delayMs = 0, speedMs = 18 }: { text: string; delayMs?: number; speedMs?: number }) {
  const skipAnimation = _hasShownTypewriter.current;
  const [displayed, setDisplayed] = useState(skipAnimation ? text : '');

  useEffect(() => {
    if (skipAnimation) return;
    sessionStorage.setItem('tw_shown', '1');
    _hasShownTypewriter.current = true;
    let index = 0;
  ...
  }, [text, delayMs, speedMs, skipAnimation]);
    ...
    <div className="space-y-8 lg:space-y-10">
      <section className="grid gap-6 xl:grid-cols-[1.5fr_0.85fr]">
            ...
            <div className="max-w-2xl">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-[6px] border border-border bg-input px-4 py-2.5 text-sm text-foreground transition-colors hover:bg-surface-container-low focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                  style={{ fontWeight: 510 }}
                >
                  <BookOpen size={16} />
        ...
        <motion.aside
          ...
          className="space-y-4"
        >
          <div className="surface-primary-card rounded-[12px] p-5 backdrop-blur">
            ...
            <div className="mt-5 flex -space-x-3">
              ...
              {activeUsers.length === 0 && (
                <div className="surface-nested-card-strong rounded-full px-4 py-3 text-sm text-on-surface-variant">
            ...
            <div className="mt-4 grid gap-3">
              <div className="surface-nested-card rounded-[8px] p-4">
              ...
              <div className="rounded-[8px] bg-accent/60 p-4">
          ...
          {profile?.status === 'approved' && myRankEntry && myRank && (
            <div className="surface-primary-card rounded-[12px] border-primary/15 p-5 dark:border-primary/20">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Recognition</p>
                  <h3 className="mt-1 text-2xl font-[590] text-on-surface">You&apos;re #{myRank} this cycle</h3>
                  <p className="mt-1 text-sm text-on-surface-variant">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
                <span className="rounded-[9999px] border border-border bg-input px-3 py-1 text-[11px] text-foreground" style={{ fontWeight: 510 }}>
              ...
              <div className="surface-nested-card mt-4 rounded-[8px] p-4">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.8fr]">
        <motion.div
          ...
          className="surface-primary-card rounded-[12px] p-6"
        >
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-[590] text-on-surface">Fresh from the hub</h3>
          ...
          {latestActivity.length === 0 ? (
            <div className="surface-nested-card mt-6 flex flex-col items-center gap-3 rounded-[12px] py-16 text-center">
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3">
                        <span className={`rounded-[4px] px-2 py-0.5 text-[11px] font-[590] tracking-wide ${entry.chipClass}`}>
                          {entry.typeLabel}
                        </span>
        ...
        <motion.aside
          ...
          className="space-y-4"
        >
          <div className="surface-primary-card rounded-[12px] p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
              ...
              <div className="rounded-[8px] bg-accent/70 p-3 text-accent-foreground">
            ...
            <div className="mt-5 space-y-3">
              {topContributors.length === 0 ? (
                <div className="surface-nested-card rounded-[8px] p-4 text-sm text-on-surface-variant">
                  Once people start posting, the most helpful contributors will show up here.
                </div>
              ) : (
                topContributors.map((entry, index) => (
                  <div key={entry.user.uid} className="surface-nested-card flex items-center gap-3 rounded-[8px] p-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-[8px] bg-primary/10 text-sm font-bold text-primary">
                      {index + 1}
                    </div>
                    <div className="min-w-0 flex-1">
          ...
          <div className="surface-primary-card rounded-[12px] p-5">
            ...
            <button
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-[6px] bg-primary px-4 py-2 text-sm text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
              style={{ fontWeight: 510 }}
            >
              Start with a tip
              <ArrowRight size={16} />
